...
cocos2dx-better的代码已经合并完成, 目前已经用在我的游戏项目上, 没有问题. 正在折腾lua支持, 看了看quick, 它写了套脚本用来自动生成lua绑定, 不过它那个Generator的可执行文件的代码在哪里? 我确实没找到啊, 貌似quick不是完全开源的, 也找不到它的代码库放在什么地方. 

不过他那套脚本看来已经写了蛮久, 还挺复杂的, 想了想, 我还是用传统tolua pkg的方式吧, 只不过我的思路和quick一样, 也得写个工具才行, 一个个写tolua的pkg真是有点累. 这个工具已经写了一半了, 在tools下面, 也是用clang自动转.
