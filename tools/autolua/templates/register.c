...
#if len($parents) > 0
    tolua_cclass(tolua_S, "${class_name}", "${lua_type}", "${generator.to_lua_type($parents[0].qualified_name, $parents[0].qualified_ns)}", NULL);
#else
    tolua_cclass(tolua_S, "${class_name}", "${lua_type}", "", NULL);
#end if
